<template>
  <section class="contactcharacter" id="about">
    <div class="contentwrapper" id="contact">
      <div class="character">
        <div class="body"></div>
        <div :class="['arm', { active: isActive, inactive: !isActive }]"></div>
        <div :class="['head', { active: isActive, inactive: !isActive }]">
          <div class="eyes"></div>
        </div>
        <div class="mug">
          <div class="steam"></div>
        ...
        <div class="contactme" @click="toggleForm">
          <div class="arrow"></div>
        </div>
        <div class="thoughts">
          <div class="abouttype">
            <p>My name is Sushant Verma. I am a...</p>
            <div class="imacontainer"><span class="ima">{{ currentText }}</span></div>
            ...
            <span class="imawhat" v-for="(text, index) in texts" :key="index">{{ text }}</span>
          </div>
    ...
    <div class="contact" :class="{ active: isActive }">
      <span class="closer" @click="closeContact"></span>
      <form @submit.prevent="submitForm">
        <input class="input name" v-model="form.name" name="name" type="text" placeholder="name">
        <input class="input email" v-model="form.email" name="email" type="email" placeholder="email">
        <textarea class="input message" v-model="form.message" name="message" placeholder="my awesome message!"></textarea>
...
export default {
  data() {
    return {
      ...
      isActive: false,
      texts: ['web designer', 'wordpress plugin author', 'web developer', 'animator'],
      currentText: '',
      textIndex: 0
    };
  },
  ...
  mounted() {
    this.updateText();
    this.textInterval = setInterval(this.updateText, 1000);
  }
...
/* Add specific colors for other links if needed */
